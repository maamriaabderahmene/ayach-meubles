...
export default function Footer() {
  ...
  const handleNewsletter = (e: React.FormEvent) => {
    ...
    if (email) {
      ...
      setEmail("");
      alert(locale === "ar" ? "شكراً! سنتواصل معك قريباً." : "Merci ! Nous vous contacterons bientôt.");
    }
  };
    ...
    <footer className="bg-[#0F0F0F] text-white">
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <div>
              <h3 className="font-sans text-lg font-semibold text-white mb-1">
                {locale === "ar" ? "اشترك في نشرتنا" : "Abonnez-vous à notre newsletter"}
              </h3>
              <p className="text-sm text-white/50">
                {locale === "ar" ? "أحدث التصاميم والعروض لك أولاً" : "Nouveautés et offres exclusives en avant-première"}
              </p>
            </div>
            <form onSubmit={handleNewsletter} className="flex w-full md:w-auto gap-0">
              <input
                ...
                placeholder={locale === "ar" ? "بريدك الإلكتروني" : "Votre e-mail"}
                className="flex-1 md:w-64 px-4 py-3 text-sm bg-white/10 border border-white/20 text-white
                           placeholder-white/40 focus:outline-none focus:border-white/60 transition-colors"
                required
              />
              <button
                type="submit"
      ...
      <div className="container py-12 md:py-16">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-10">
          ...
          {/* Quick links */}
          <div>
            <h4 className="text-xs font-sans font-semibold uppercase tracking-[0.18em] text-white/60 mb-5">
            ...
            <ul className="space-y-3">
              {[
                ...
                <li key={href}>
                  <Link href={href} className="text-sm text-white/50 hover:text-white transition-colors link-animated">
                    {label}
                  </Link>
                </li>
              ))}
